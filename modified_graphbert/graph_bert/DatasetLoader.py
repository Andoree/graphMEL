...
from graph_bert.base_class.dataset import dataset
import torch
import numpy as np
...
class DatasetLoader(dataset):
    ...
    def load(self):
        ...
        if self.dataset_name == 'cora':
            ...
            idx_val = range(5, 10)
            idx_test = range(5, 10)
        elif 'wordnet' in self.dataset_name:
            idx_train = range(5)
            idx_val = range(5, 10)
            idx_test = range(5, 10)
        ...
        result = {'X': features, 'A': adj, 'S': eigen_adj, 'index_id_map': index_id_map, 'edges': edges_unordered,
                  'raw_embeddings': raw_embeddings, 'wl_embedding': wl_embedding, 'hop_embeddings': hop_embeddings,
                  'int_embeddings': int_embeddings, 'y': labels, 'idx': idx, 'idx_train': idx_train,
                  'idx_test': idx_test, 'idx_val': idx_val}
        for i in result:
            if isinstance(result[i], torch.Tensor):
                result[i] = result[i].to(self.device)
        return result
