...
from graph_bert.MethodGraphBert import MethodGraphBert
...
class MethodGraphBertGraphRecovery(BertPreTrainedModel):
    ...
    def __init__(self, config, pretrained_path=None, device='cuda'):
        super(MethodGraphBertGraphRecovery, self).__init__(config)
        self.config = config
        self.bert = MethodGraphBert(config).to(device)
        if pretrained_path is not None:
            print("Load pretrained model from {}".format(pretrained_path))
            self.bert.from_pretrained(pretrained_path).to(device)
        self.init_weights()
    ...
    def predict(self, raw_features, wl_role_ids, init_pos_ids, hop_dis_ids, idx=None):
