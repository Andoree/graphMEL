...
from predict import find_bpe_position_by_offset
...
def bpe_tokenize(bpe_tokenizer, sentence):
    ...
    for token in sentence:
        ...
        sent_bpe_offsets += [(token.start_position, token.end_position) for _ in range(len(token_bpes))]
...
class MaskedTokenPredictorBert:
    def __init__(self, model, bpe_tokenizer, max_len=300, mask_in_multiunit=False, device='cpu'):
        self._model = model
    ...
    def __call__(self, sentences, masked_position, 
                 mask_token=True, 
                 n_top=5, 
                 n_units=1, 
                 n_tokens=[1],
        ...
        for sent, mask_pos in zip(sentences, masked_position):
            ...
            masked_position = find_bpe_position_by_offset([bpe_offsets], 
                                                          (sent[mask_pos].start_position, 
                                                           sent[mask_pos].end_position)) 
        ...
        result_preds = [[] for _ in range(len(sentences))]
        result_scores = [[] for _ in range(len(sentences))]
        if 1 in n_tokens:
    ...
    def predict_single_unit(self, bpe_tokens, masked_position, 
        ...
        segments_ids = np.zeros_like(token_ids, dtype=int)
        segments_tensor = torch.tensor(segments_ids).to(self.device)
        
        target_sent = self._model.predict_from_hidden_state(tokens_tensor, masked_position, embedding,
                                  attention_masks_tensor).detach().cpu().numpy()

        final_top_scores = []
        final_top_tokens = []
        for i in range(target_sent.shape[0]):
            top_ids = nlargest_indexes(target_sent[i][masked_position[i] + 1], n_top)
    ...
    def predict_single_word(self, bpe_tokens, masked_position, 
        ...
        pred_tokens, scores = self.predict_single_unit(bpe_tokens, 
                                                       masked_position, 
                                                       mask_token=mask_token, 
                                                       n_top=n_top, embedding=embedding)
        ...
        for j in range(len(pred_tokens)):
            if n_units > 1:
                ...
                seq_list = self.generate_multiunit_token(masked_position[j], bpe_tokens[j],
                                                         ...
                                                         n_units=n_units, embedding=embedding)
    ...
    def generate_from_tail(self, pred, bpe_tokens, masked_position, 
        ...
        while (pred[0] == '#') and (it < max_subunits):
            ...
            preds, pred_scores = self.predict_single_unit([bpe_tokens], [masked_position], 
                                                          n_top=n_top, 
                                                          mask_token=False, embedding=embedding)
            pred = preds[0][0]
            result.append(pred)
