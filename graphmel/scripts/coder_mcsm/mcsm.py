...
def mcsm(embedding_list, embedding_type_list, umls_dir, type_list=t_list, k=40, lang_range=['ENG'],
    ...
    umls = UMLS(umls_dir, source_range='SNOMEDCT_US',
...
def main(args):
    bert_encoder_paths = args.bert_encoder_paths
    umls_dir = args.umls_dir

    # mcsm(embedding_list, ["bert"] * 6, check_intersection=True)
    # mcsm(embedding_list, ["bert"] * 6)
    mcsm(bert_encoder_paths, ["bert"] * len(bert_encoder_paths), umls_dir=umls_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bert_encoder_paths', nargs='+', type=str)
