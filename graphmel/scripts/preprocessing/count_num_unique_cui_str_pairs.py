...
def main():
    ...
    gb_cui_str_lat_df = df_mrconso_unique_cui_str_lat.groupby(['CUI', 'STR', 'LAT'])['STR'].count()
    gb_cui_str_lat_path = os.path.join(output_dir, "gb_cui_str_lat.tsv")
    gb_cui_str_lat_df.to_csv(gb_cui_str_lat_path, sep='\t', index=False)

    df_mrconso_unique_cui_str = df_mrconso[["CUI", "STR"]].drop_duplicates(subset=("CUI", "STR"), inplace=False)

    gb_cui_str_df = df_mrconso_unique_cui_str.groupby(['CUI', 'STR'])['STR'].count()
    gb_cui_str_path = os.path.join(output_dir, "gb_cui_str.tsv")
    gb_cui_str_df.to_csv(gb_cui_str_path, sep='\t', index=False)
