...
def evaluate_single_checkpoint_acc1_acc5(checkpoint_path: str, vocab, entity_texts, labels):
    bert_ranker = BERTRanker(checkpoint_path, vocab)
    ...
    acc_1 = np.mean(correct_top1)
    acc_5 = np.mean(correct_top5)
        ...
        if os.path.isdir(model_setup_dir):
            for checkpoint_name in os.listdir(model_setup_dir):
                ...
                if os.path.isdir(checkpoint_dir):
                    ...
                    evaluation_dict = {"Model setup": model_setup_name, "Checkpoint name": checkpoint_name,
                                       "Acc@1": acc_1, "Acc@5": acc_5}
                    evaluation_results.append(evaluation_dict)
                    logging.info(f"Finished processing checkpoint: {checkpoint_dir}, Acc@1: {acc_1}, Acc@5 : {acc_5}")
                else:
                    pass
        else:
            pass
