...
def rgcn_sapbert_train_step(model: RGCNSapMetricLearning, batch, amp, device):
    ...
    if amp:
        with autocast():
            text_loss, graph_loss, intermodal_loss = \
                model(term_1_input_ids=term_1_input_ids, term_1_att_masks=term_1_att_masks,
...
def train_rgcn_sapbert(model: RGCNSapMetricLearning, train_loader: PositivePairNeighborSampler,
    ...
    for batch in pbar:
        optimizer.zero_grad()
        sapbert_loss, graph_loss, intermodal_loss = rgcn_sapbert_train_step(model=model, batch=batch, amp=amp,
                                                                            device=device)
...
def main(args):
    ...
    output_subdir = f"rgcn_{'.'.join((str(x) for x in args.rgcn_num_neighbors))}_{args.rgcn_num_hidden_channels}-" \
                    f"{args.rgcn_num_outer_layers}-{args.rgcn_num_inner_layers}-{args.rgcn_num_bases}" \
                    f"-{args.rgcn_num_blocks}_{args.use_rel_or_rela}_remove_loops_{args.remove_selfloops}" \
                    f"_text_loss_{args.text_loss_weight}_intermodal_{args.modality_distance}" \
                    f"_{args.intermodal_loss_weight}_graph_{args.graph_loss_weight}_" \
                    f"_lr_{args.learning_rate}_b_{args.batch_size}_{conv_type}_drop_{args.rgcn_dropout_p}"
    ...
    model = RGCNSapMetricLearning(bert_encoder=bert_encoder, rgcn_num_hidden_channels=args.rgcn_num_hidden_channels,
